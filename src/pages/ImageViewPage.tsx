...
export default function ImageViewPage() {
  ...
  if (loading) {
    return (
      <div className="flex flex-col items-center justify-center min-h-[50vh] space-y-4">
        ...
        <p className="text-primary font-bold">{t('imageView.loading')}</p>
      </div>
    );
  }
  ...
  if (error || !imageDetails) {
    return (
      ...
      >
        ...
        <h2 className="text-2xl font-serif font-black text-error">{t('imageView.errorTitle')}</h2>
  ...
  const formattedDate = createdAt
    ? new Date(createdAt).toLocaleDateString(i18n.language === 'pt-BR' ? 'pt-BR' : 'en-US')
    : null;
        ...
        <div className="w-full md:w-1/3 p-6 flex flex-col space-y-6">
            ...
            <div className="flex flex-col gap-2">
              <div className="flex items-center gap-2 text-xs text-on-surface-variant bg-surface-container-low px-3 py-1.5 rounded-lg border border-outline-variant/30 w-fit">
                ...
                <span className="font-bold">
                  {isAnonymous ? t('imageView.sentByAnonymous') : t('imageView.sentByAuthenticated')}
                </span>
              </div>
                ...
                <div className="flex items-center gap-4 text-[11px] font-mono font-bold text-on-surface-variant">
                  {formattedDate && (
                    <span className="flex items-center gap-1">
                      ...
                      {t('imageView.uploadLabel')} {formattedDate}
                    </span>
                  )}
                  {formattedSize && (
                    <span className="flex items-center gap-1">
                      <HardDrive className="w-3 h-3 text-primary" />
                      {t('imageView.sizeLabel')} {formattedSize}
                    </span>
                  )}
                </div>
              )}
          ...
          <div className="space-y-3">
            <h3 className="text-xs font-extrabold text-primary uppercase tracking-wide flex items-center gap-1.5">
              ...
              {t('imageView.tagsTitle')}
            </h3>
            {tags && tags.length > 0 ? (
              <div className="flex flex-wrap gap-2">
                {tags.map((tag) => (
                  <span key={tag} className="px-2.5 py-1 bg-secondary-container text-on-secondary-container text-[11px] font-bold rounded-md">
                    #{tag}
                  </span>
                ))}
              </div>
            ) : (
              <p className="text-xs text-on-surface-variant italic">{t('imageView.noTags')}</p>
            )}
          </div>
              ...
              {timeLeft ? (
                <div className="flex gap-2">
                  <div className="flex flex-col items-center bg-tertiary-container text-on-tertiary-container p-2 rounded-lg min-w-[50px] shadow-sm">
                    ...
                    <span className="text-[9px] font-bold uppercase">{t('imageView.days')}</span>
                  </div>
                  <div className="flex flex-col items-center bg-tertiary-container text-on-tertiary-container p-2 rounded-lg min-w-[50px] shadow-sm">
                    <span className="font-mono font-black text-lg">{timeLeft.hours}</span>
                    <span className="text-[9px] font-bold uppercase">{t('imageView.hours')}</span>
                  </div>
                  <div className="flex flex-col items-center bg-tertiary-container text-on-tertiary-container p-2 rounded-lg min-w-[50px] shadow-sm">
                    <span className="font-mono font-black text-lg">{timeLeft.minutes}</span>
                    <span className="text-[9px] font-bold uppercase">{t('imageView.minutes')}</span>
                  </div>
                  <div className="flex flex-col items-center bg-error-container text-on-error-container p-2 rounded-lg min-w-[50px] shadow-sm animate-pulse">
                    ...
                    <span className="text-[9px] font-bold uppercase">{t('imageView.seconds')}</span>
                  </div>
                </div>
              ) : (
                <p className="text-error font-bold text-sm">{t('imageView.timeExpired')}</p>
              )}
            </div>
          )}
          ...
          <div className="flex-grow flex flex-col justify-end pt-4 border-t border-outline-variant/30">
            ...
            >
              ...
              <span>{t('imageView.downloadPortrait')}</span>
            </a>
          </div>
        </div>
      </div>
